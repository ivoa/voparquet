...
That \xmlel{TABLE} element is the one whose metadata describes
the data in the enclosing parquet file,
and it is denoted here the {\em metadata table}.
The VOTable document may contain other \xmlel{TABLE} elements
providing auxiliary data or metadata,
but these are not relevant to the VOParquet convention.

The metadata table looks exactly like a normal \xmlel{TABLE} element
